  ...
  return (
    <div className="fixed inset-0 z-40 bg-background overflow-y-auto animate-slide-up">
      ...
      <div className="px-4 py-5 pb-32">
        ...
        {/* Smile ID Verified Badge — tappable */}
        {property.verified && (
          <button
            onClick={() => setShowSmileIDDetails(true)}
            className="w-full text-left p-4 rounded-2xl bg-primary/5 border border-primary/15 mb-4 active:scale-[0.98] transition-transform"
          >
        ...
        {/* Smile ID Verification Details Modal */}
        {showSmileIDDetails && (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-foreground/40 backdrop-blur-sm animate-fade-in" onClick={() => setShowSmileIDDetails(false)}>
            <div className="w-full max-w-md bg-card rounded-t-3xl p-5 pb-8 animate-slide-up" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex items-center gap-3 mb-5">
                <div className="w-12 h-12 rounded-xl gradient-trust flex items-center justify-center">
                  <svg viewBox="0 0 24 24" className="w-6 h-6 text-primary-foreground" fill="none">
                    <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" />
                    <path d="M7 14c0 0 2 3.5 5 3.5s5-3.5 5-3.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
                    <circle cx="9" cy="10" r="1.2" fill="currentColor" />
                    <circle cx="15" cy="10" r="1.2" fill="currentColor" />
              ...
              <div className="p-3 rounded-xl bg-primary/10 border border-primary/20 mb-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground">Verified on {new Date(Date.now() - 3 * 86400000).toLocaleDateString("en-KE", { day: "numeric", month: "short", year: "numeric" })}</p>
                  </div>
              ...
              <div className="space-y-2 mb-5">
                {[
                  { label: "Image authenticity", detail: "Original photos confirmed", passed: true },
                  { label: "AI-generated content", detail: "No AI generation detected", passed: true },
                  { label: "Reverse image search", detail: "No stock photo matches", passed: true },
                  { label: "Metadata & EXIF", detail: "Valid GPS & timestamp data", passed: true },
                  { label: "Duplicate detection", detail: "No duplicates found", passed: true },
                  { label: "Photo quality", detail: "High resolution (4032×3024)", passed: true },
                ].map((check) => (
              ...
              <div className="p-3 rounded-xl bg-secondary mb-4">
                <div className="grid grid-cols-2 gap-3 text-[10px]">
                  <div>
                    ...
                    <p className="font-semibold font-mono">SID-{property.id.slice(0, 8).toUpperCase()}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Photos Checked</p>
                    <p className="font-semibold">{property.images?.length || 4} photos</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Valid Until</p>
                    <p className="font-semibold">{new Date(Date.now() + 87 * 86400000).toLocaleDateString("en-KE", { day: "numeric", month: "short", year: "numeric" })}</p>
                  </div>
                  <div>
              ...
              <button
                onClick={() => setShowSmileIDDetails(false)}
