...
const SplashScreen = ({ onComplete }: { onComplete: () => void }) => {
  ...
  const bg = isDark
    ...
    : "linear-gradient(180deg, hsl(150 60% 26%) 0%, hsl(150 60% 14%) 100%)";
  const fg = isDark ? "hsl(148 52% 48%)" : "white";
  const fgFill = isDark ? "hsl(148, 52%, 48%)" : "white";
  const houseCut = isDark ? "hsl(150, 20%, 7%)" : "hsl(150, 60%, 20%)";
  const skylineFill = isDark ? "hsl(148 52% 48%)" : "white";
      ...
      <div className="absolute bottom-0 left-0 right-0 overflow-hidden" style={{ height: "18%" }}>
        ...
        >
          ...
          {[42, 92, 132, 152, 227, 252, 297, 317, 342].map((x, i) => (
            <rect key={i} x={x - 2} y={x === 152 ? 33 : x === 252 ? 30 : x === 342 ? 35 : 43} width="4" height="4" fill={houseCut} opacity="0.3" rx="0.5" />
          ))}
        </svg>
      </div>
      ...
      <div className="relative flex flex-col items-center">
        <div className="relative w-32 h-32 mb-6">
          <svg viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
            ...
            <circle
              cx="60" cy="52" r="38"
              stroke={fg} strokeWidth="5" fill="none"
            ...
            >
              <path d="M60 25 L40 42 L80 42 Z" fill={fgFill} />
              <rect x="70" y="28" width="5" height="10" fill={fgFill} />
              <rect x="44" y="42" width="32" height="24" fill={fgFill} />
              <rect x="55" y="50" width="10" height="16" fill={houseCut} rx="1" />
              <rect x="48" y="47" width="6" height="6" fill={houseCut} rx="0.5" />
              <rect x="66" y="47" width="6" height="6" fill={houseCut} rx="0.5" />
            </g>
        ...
        {/* Brand name — phase 4 */}
        <h1
          className={`text-4xl font-extrabold tracking-tight transition-all duration-600 ${textClass}`}
          style={{
            opacity: phase >= 4 ? 1 : 0,
        ...
        <p
          className={`mt-1 text-xs font-medium tracking-widest uppercase transition-all duration-600 ${subtitleColor}`}
          style={{
            opacity: phase >= 5 ? 1 : 0,
